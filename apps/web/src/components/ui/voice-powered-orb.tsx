...
export function VoicePoweredOrb({
  ...
  useEffect(() => {
    ...
    const tick = (t: number) => {
      ...
      const hover = active ? 0.35 + Math.min(intensity, 1) * 0.65 : 0.08;
      const hoverIntensity = active ? 0.35 + Math.min(intensity, 1) * 0.45 : 0.12;
      const speed = active ? 0.55 + Math.min(intensity, 1) * 1.2 : 0.2;
